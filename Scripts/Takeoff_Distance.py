...
def DefineAndTrainNN(Dataset, MainPath):
    ...
    checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
                filepath = os.path.join(MainPath, 'Out/Takeoff Distance Predictions/Checkpoints/checkpoint'),
...
def PlotOutputs(MainPath, history, Dataset):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(MainPath, 'Out/Takeoff Distance Predictions/Figures/Error_Epoch.jpg'), dpi = 600)
    plt.close()
